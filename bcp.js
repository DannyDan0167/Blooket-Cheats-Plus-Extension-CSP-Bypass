(async () => {
    ...
    let C = {
        ...
        extras: [{
            name: "Fetch latest GUI",
            description: "Fetches the lastest version of BCP",
            run: function() {
                const url = "https://raw.githubusercontent.com/DannyDan0167/Blooket-Cheats-Plus/main/GUI/Gui.js";
            ...
            name: "Blooket Bot",
            description: "Opens the Blooket Bot by cryptodude3",
            run: function() {
                ...
                title.textContent = 'Blooket Bot (cryptodude3)';
                title.style.flex = '1';
                ...
                resizeHandles.forEach(function(handle) {
                    ...
                    resizeHandle.className = handle.class;
                    ...
                    if (handle.class.includes('top')) resizeHandle.style.top = '-5px';
                    if (handle.class.includes('bottom')) resizeHandle.style.bottom = '-5px';
                    if (handle.class.includes('left')) resizeHandle.style.left = '-5px';
                    if (handle.class.includes('right')) resizeHandle.style.right = '-5px';

                    if (handle.class.includes('top') || handle.class.includes('bottom')) {
                        resizeHandle.style.width = '100%';
                        resizeHandle.style.height = '10px';
                    }
                    if (handle.class.includes('left') || handle.class.includes('right')) {
                        resizeHandle.style.width = '10px';
                    ...
                    resizeHandle.addEventListener('mousedown', function(e) {
                        ...
                        resizeStartWidth = parseInt(document.defaultView.getComputedStyle(container).width, 10);
                        resizeStartHeight = parseInt(document.defaultView.getComputedStyle(container).height, 10);
                        document.body.style.userSelect = 'none';
                    });
                    ...
                    document.addEventListener('mousemove', function(e) {
                        if (isResizing) {
                            ...
                            if (handle.class.includes('right')) {
                                newWidth = resizeStartWidth + (e.clientX - resizeStartX);
                                container.style.width = newWidth + 'px';
                            }
                            if (handle.class.includes('bottom')) {
                            ...
                            if (handle.class.includes('left')) {
                                newWidth = resizeStartWidth - (e.clientX - resizeStartX);
                                container.style.width = newWidth + 'px';
                                container.style.left = (container.offsetLeft + (e.clientX - resizeStartX)) + 'px';
                            }
                            if (handle.class.includes('top')) {
                                newHeight = resizeStartHeight - (e.clientY - resizeStartY);
                                container.style.height = newHeight + 'px';
                                container.style.top = (container.offsetTop + (e.clientY - resizeStartY)) + 'px';
                            }
                        }
                    });
